...
@Composable
internal fun ChooseTranslationExerciseView(
    actionSender: ActionSender<Action>,
    learnState: LearnState.ChooseTranslationExercise,
) {
    Column {
        Text("ChooseTranslationExercise: ${learnState.learnUnit}")
        Text("Answers: ${learnState.answers}")
        Button(
            onClick = {
                actionSender.sendAction(LearnAction.SubmitAnswer(learnState.learnUnit.baseLanguageText))
            },
        ) {
